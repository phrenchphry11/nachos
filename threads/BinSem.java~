...
public class BinSem {
    ...
    public BinSem(int initialValue) {
        Lib.assertTrue(initialValue==0 || initialValue==1);
	value = initialValue;
    }
	...
	public void run() {
	    for (int i=0; i<5; i++) {
                System.out.println("spawned thread with i="+i+" ping.P()");
		ping.P();
                System.out.println("spawned thread with i="+i+" pong.V()");
		pong.V();
	    }
	}
	...
	for (int i=0; i<5; i++) {
	    System.out.println("main thread with i="+i+" ping.V()");
	    ping.V();
            System.out.println("main thread with i="+i+" pong.P()");
	    pong.P();
	}
    }
